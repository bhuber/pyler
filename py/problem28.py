...
def problem28(n):
    ...
    g(n) = f(n) + f(n - 2) + ... + f(1)
         ...
         = 1 + sum(4 * (2i)^2 + 10 * (2i + 1), i, i = 1, i < floor(n/2))
    ...
    ss(n) = n * (n + 1) * (2 * n + 1) / 6
    let ses(n) = sum of even squares <= n = sum((2*i)^2, i = 1, i <= n/2) = 4 * sum(i^2, i = 1, i <= n/2) 
             = 4 * ss(n / 2)
    let sos(n) = sum of odd squares <= n = ss(n) - ses(n)
    let e(n) = sum of even i <= n = 2 * s(n / 2)
